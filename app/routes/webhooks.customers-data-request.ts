...
import { recordCustomerDataRequest } from "../services/privacy.server";
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  const result = await recordCustomerDataRequest({ db, shopDomain: shop, webhookId: webhookEventId(request, topic, shop, rawBody) });
