...
- **`/etc/vps-apps/skuforge.env` has CRLF line endings.** An unstripped `\r` lands in the
  `Authorization` header and `curl` fails with exit 43. The script pipes both values through
  `tr -d '\r'`; do not remove that. The trailing `xargs` only trims whitespace, so a secret
  containing quotes or backslashes would be mangled — keep `CRON_SECRET` alphanumeric.
