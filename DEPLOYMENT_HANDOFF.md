...
- **Deployment artifacts:** `docker-compose.yml` now supplies `web` + `postgres:16-alpine`, a persistent database volume, a database health gate, and Traefik labels. Copy `env.production.example` to the VPS-only `.env`, populate it through the server secret workflow, then `chmod 600 .env`.
