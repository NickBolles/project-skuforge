...
- **Deployment artifacts:** `docker-compose.yml` now supplies `web` + `postgres:16-alpine`, a persistent database volume, a database health gate, and Traefik labels. Copy `env.production.example` to the VPS-only `.env`, populate it through the server secret workflow, then `chmod 600 .env`. `DATABASE_URL` must point at `db` and URL-encode any reserved password characters.
- **Credential rotation:** `POSTGRES_PASSWORD` initializes Postgres only on its first boot. To rotate it later, run `ALTER ROLE skuforge PASSWORD '...'` against the existing database, update the URL-encoded password in `DATABASE_URL`, then restart the web service; changing `.env` alone will lock the app out.
- **PostgreSQL migration boundary:** production uses `prisma/postgres/schema.prisma` and its own `prisma/postgres/migrations/` history (SQLite development migrations are deliberately separate). The Docker image generates the PostgreSQL client, and container start runs `npm run setup:postgres` before serving. Validate the exact production migration history against a disposable PostgreSQL database before the VPS release.
